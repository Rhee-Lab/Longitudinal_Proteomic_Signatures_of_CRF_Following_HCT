...
plot_combined_qq <- function(reg_list,
                             base_dir,
                             filename = "QQ_All_Contrasts.png") {
  
  qq_list <- lapply(names(reg_list), function(lbl) {
    reg <- reg_list[[lbl]]
    if (is.null(reg)) return(NULL)
    pvals <- sort(reg$p[!is.na(reg$p) & is.finite(reg$p)])
    if (length(pvals) < 10) return(NULL)
  ...
  df <- bind_rows(qq_list)
  
  p <- ggplot(df, aes(expected, observed, color = Contrast)) +
    geom_point(alpha = .5) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
    theme_bw() +
    labs(
      title = "Combined QQ – All Contrasts",
      x = "Expected -log10(p)",
      y = "Observed -log10(p)"
    )
  
  ggsave(file.path(base_dir, paste0(filename)),
...
run_metabolite_class_ORA <- function(reg_df, class_sets, cmpd_col_name, pval_col_name, beta_col_name) {  
  # Extract metabolite groups
  all_mets <- unique(reg_df[[cmpd_col_name]])
  up_mets <- reg_df %>%
    filter(.data[[pval_col_name]] < 0.05, .data[[beta_col_name]] > 0) %>%
    pull(.data[[cmpd_col_name]]) %>%
  ...
  # Helper function to perform ORA for a given direction
  perform_ora <- function(sig_mets, all_mets, class_sets, direction) {
    sig_mets <- unique(sig_mets)
    all_mets <- unique(all_mets)
    if(length(sig_mets) < 3) return(NULL)

    res <- purrr::map_df(names(class_sets), function(cls) {
      members <- class_sets[[cls]]
      a <- sum(sig_mets %in% members)           # sig in class
      b <- length(sig_mets) - a                 # sig not in class
      c <- sum(all_mets %in% members) - a       # non-sig in class
      d <- length(all_mets) - a - b - c         # non-sig not in class

      if (any(c(a,b,c,d) < 0)) {
        return(tibble(
          compound_class_library = cls,
          a = a, b = b, c = c, d = d,
          fisher_p = NA_real_
        ))
      }
      ...
      # Use one-sided test (greater)
      # test for enrichment in the sig group, rather than including sig. depletion
      fisher_p <- fisher.test(matrix(c(a, b, c, d),
                                     nrow = 2,
                                     byrow = TRUE),
                              alternative = "greater")$p.value

      # Calculate odds ratio: (a/b) / (c/d) = (a*d) / (b*c)
      odds_ratio <- if (b == 0 || c == 0) {
        NA_real_ # Avoid division by zero; interpret as infinite enrichment
      } else {
        (a * d) / (b * c)
      }
      ...
      mutate(
        neglog10_fisher = -log10(fisher_p),
        enrichment      = a / (a + c),
        direction       = direction
      ) %>%
      arrange(fisher_p)
  ...
  # Run ORA for both directions
  res_ora_up <- perform_ora(up_mets, all_mets, class_sets, "UP")
  res_ora_down <- perform_ora(down_mets, all_mets, class_sets, "DOWN")
  
  # Return as list
  return(list(
    ORA_UP = res_ora_up,
    ORA_DOWN = res_ora_down
  ))
}

run_metabolite_class_MSEA <- function(reg_df, class_sets, cmpd_col_name, msea_ranking_col_name) {
  # Use t-test "statistic" (beta/std_error(beta)) to rank metabolites for MSEA
  ranked <- reg_df %>%
    filter(is.finite(.data[[msea_ranking_col_name]])) %>%
    arrange(desc(.data[[msea_ranking_col_name]])) %>%
    dplyr::select(!!sym(cmpd_col_name), !!sym(msea_ranking_col_name)) %>%
    tibble::deframe()

  if (length(class_sets) == 0) {
    return(NULL)
  }

  # Run fgseaMultilevel for MSEA
  set.seed(random_seed)
  msea_res <- fgseaMultilevel(
    pathways = class_sets,
    stats = ranked,
    minSize = 5,
    maxSize = 500
...
run_metabolite_class_enrichment_per_contrast <- function(reg_df, metab_mapped, id_col, contrast, dir, cmpd_col_name = "feature_label", pval_col_name = "p", beta_col_name = "beta", filename_suffix = "", run_type="Linear", annot_col_names = c("compound_class_library", "kegg_map_ids"), use_t_statistic = FALSE) {
  print(paste("Running class enrichment for contrast:", contrast))

  # Combine data and annotations; drop annotation columns already in reg_df to avoid duplicates
  cols_to_drop <- setdiff(colnames(metab_mapped), id_col)
  reg_annot <- reg_df %>%
    dplyr::select(-any_of(cols_to_drop)) %>%
    left_join(metab_mapped, by = id_col)

  # Sort by ascending p-value
  reg_annot <- reg_annot %>%
    arrange(!!sym(pval_col_name))

  # Export annotated regression results
  write.csv(reg_annot, file.path(dir, paste0(run_type, "_Metabolites_Results_Annotated_", contrast, filename_suffix, ".csv")), row.names = FALSE)

  results_by_annot <- list()

  # Run ORA and MSEA per annotation type
  for (annot_col in annot_col_names) {

    # Build annotation sets, filtering out NA and placeholder "-" values
    tmp <- reg_annot %>%
      dplyr::select(!!sym(annot_col), !!sym(cmpd_col_name)) %>%
      filter(!is.na(!!sym(annot_col)), !!sym(annot_col) != "-")

    # For ";" separated annotations (e.g. kegg_id, kegg_map_ids), expand into one row per entry
    if (annot_col %in% c("kegg_id", "kegg_map_ids")) {
      tmp <- tidyr::separate_rows(tmp, !!sym(annot_col), sep = ";\\s*")
    }

    annot_sets <- tmp %>%
      group_by(!!sym(annot_col)) %>%
      summarise(members = list(unique(!!sym(cmpd_col_name)))) %>%
      tibble::deframe()

    # Run ORA
    ora_results <- run_metabolite_class_ORA(reg_annot, annot_sets, cmpd_col_name, pval_col_name, beta_col_name)

    # Run MSEA
    if (!use_t_statistic) {
      reg_annot$signed_log10_p <- sign(reg_annot[[beta_col_name]]) * -log10(pmax(reg_annot[[pval_col_name]], .Machine$double.xmin))
    }
    msea_rank_col <- if (use_t_statistic) "statistic" else "signed_log10_p"
    res_msea <- run_metabolite_class_MSEA(reg_annot, annot_sets, cmpd_col_name, msea_ranking_col_name = msea_rank_col)

    # Convert leadingEdge from feature_label to human-readable compound_id_library
    if (!is.null(res_msea) && "leadingEdge" %in% colnames(res_msea) &&
        "compound_id_library" %in% colnames(reg_annot)) {
      label_to_cmpd <- reg_annot %>%
        dplyr::select(!!sym(cmpd_col_name), compound_id_library) %>%
        dplyr::distinct() %>%
        tibble::deframe()
      res_msea$leadingEdge <- lapply(res_msea$leadingEdge, function(ids) {
        mapped <- label_to_cmpd[ids]
        ifelse(!is.na(mapped) & nzchar(mapped), mapped, ids)
      })
    }

    # For kegg_map_ids, add human-readable KEGG pathway names to ORA and MSEA results
    if (annot_col == "kegg_map_ids") {
      kegg_pathway_names <- tryCatch({
        raw <- KEGGREST::keggList("pathway")
        # raw: named character vector, names = "path:map00010",
        #      values = "Glycolysis / Gluconeogenesis - Reference pathway"
        tibble::tibble(
          map_id       = sub("^path:", "", names(raw)),
          pathway_name = as.character(raw)
        )
      }, error = function(e) {
        message("Could not fetch KEGG pathway names: ", conditionMessage(e))
        tibble::tibble(map_id = character(0), pathway_name = character(0))
      })
      add_kegg_name <- function(df, id_col) {
        if (is.null(df) || nrow(df) == 0) return(df)
        df %>%
          dplyr::left_join(kegg_pathway_names, by = setNames("map_id", id_col)) %>%
          dplyr::relocate(pathway_name, .after = !!sym(id_col))
      }
      ora_results$ORA_UP   <- add_kegg_name(ora_results$ORA_UP,   "compound_class_library")
      ora_results$ORA_DOWN <- add_kegg_name(ora_results$ORA_DOWN, "compound_class_library")
      if (!is.null(res_msea)) res_msea <- add_kegg_name(res_msea, "pathway")
    }

    # Store in single xlsx with sheets: ORA_UP, ORA_DOWN, MSEA
    wb <- createWorkbook()
    addWorksheet(wb, "ORA_UP")
    addWorksheet(wb, "ORA_DOWN")
    addWorksheet(wb, "MSEA")
    if (!is.null(ora_results$ORA_UP)) {
      writeData(wb, "ORA_UP", ora_results$ORA_UP)
    } else {
      writeData(wb, "ORA_UP", data.frame(message = "Not enough significant metabolites for ORA UP"))
    }
    if (!is.null(ora_results$ORA_DOWN)) {
      writeData(wb, "ORA_DOWN", ora_results$ORA_DOWN)
    } else {
      writeData(wb, "ORA_DOWN", data.frame(message = "Not enough significant metabolites for ORA DOWN"))
    }
    if (!is.null(res_msea)) {
      writeData(wb, "MSEA", res_msea)
    } else {
      writeData(wb, "MSEA", data.frame(message = "Not enough significant metabolites for MSEA"))
    }
    saveWorkbook(wb, file.path(dir, paste0("MSEA_and_ORA_Results_", annot_col, "_", contrast, filename_suffix, ".xlsx")), overwrite = TRUE)

    results_by_annot[[annot_col]] <- list(
      ORA_UP = ora_results$ORA_UP,
      ORA_DOWN = ora_results$ORA_DOWN,
      MSEA = res_msea
    )
  }

  return(results_by_annot)
}
...
create_responder_count_data <- function(patient_data, vo2_cutoffs, timepoint) {
  # For logistic regression analysis
  plot_data <- data.frame()
  timepoint_label <- ifelse(timepoint == "6m", "6 months", "12 months")
  
  for (cutoff in vo2_cutoffs) {
    cutoff_suffix <- paste0("c", gsub("\\.", "_", as.character(cutoff)))
    col_name <- paste0("resp_", timepoint, "_", cutoff_suffix)
    
    if (col_name %in% colnames(patient_data)) {
      counts <- table(patient_data[[col_name]], useNA = "no")
      plot_data <- rbind(plot_data, data.frame(
        Cutoff = cutoff,
        Timepoint = timepoint_label,
        Response = c("Below VO\u2082peak Decline Cutoff", "Above Cutoff"),
        Count = as.numeric(c(counts["0"], counts["1"]))
      ))
    }
  }
  
  # Remove rows with NA counts
  plot_data <- plot_data[!is.na(plot_data$Count), ]
  return(plot_data)
}

plot_stacked_barplot_vo2_cutoffs <- function(plot_data, output_dir, font_size = 20, filename_suffix = "") {
  # For logistic regression analysis
  p <- ggplot(plot_data, aes(x = factor(Cutoff), y = Count, fill = Response)) +
    geom_bar(stat = "identity", position = "stack") +
    facet_wrap(~ Timepoint, ncol = 2) +
    scale_fill_manual(
      values = c("Below VO\u2082peak Decline Cutoff" = "#2166AC", "Above Cutoff" = "#B2182B"),
      breaks = c("Below VO\u2082peak Decline Cutoff", "Above Cutoff")
    ) +
    labs(
      x = "VO\u2082peak Decline Cutoff (mL/kg/min)",
  ...
  ggsave(
    filename = file.path(output_dir, paste0("responder_counts_stacked_barplot", filename_suffix, ".png")),
    plot = p,
    width = 6,
    height = 6,
    dpi = 600
...
plot_msea_bar_plot <- function(msea_res, contrast, dir, padj_cutoff = 0.05, top_n = 20, font_size = 16, filename_suffix = "", color_limits = NULL) {
  if (is.null(msea_res) || nrow(msea_res) == 0) {
    message("No MSEA results available for ", contrast)
    return(NULL)
  }
  ...
  if (nrow(msea_sig) == 0) {
    message("No significant MSEA results for ", contrast, " at padj < ", padj_cutoff)
    return(NULL)
  }
  
  # Take top N by padj
  msea_plot_data <- msea_sig %>%
    head(top_n) %>%
    mutate(
      neglog10_padj = -log10(padj),
      pathway_short = ifelse(nchar(pathway) > 40,
  ...
  # Make truncated labels unique when different full pathways share the same label
  msea_plot_data <- msea_plot_data %>%
    group_by(pathway_short) %>%
    mutate(pathway_short = if (n() > 1)
      paste0(pathway_short, " (", row_number(), ")")
    else
      pathway_short
    ) %>%
    ungroup()

  # Set color scale limits for consistent coloring across plots
  if (is.null(color_limits)) {
    color_limits <- c(0, max(msea_plot_data$neglog10_padj, na.rm = TRUE))
  }
  
  # Create bar plot
  p <- ggplot(msea_plot_data, aes(x = NES, y = reorder(pathway_short, NES), fill = neglog10_padj)) +
    geom_col(width = 0.6, color = "black", linewidth = 0.5, position = "identity") +
    scale_fill_gradient(low = "white", high = "#B2182B", name = "-log10(padj)", limits = color_limits, oob = scales::squish) +
    labs(
      title = paste0("MSEA (p-adj < ", padj_cutoff, ") - ", contrast),
  ...
  # Save plot
  ggsave(
    filename = file.path(dir, paste0("MSEA_Barplot_", contrast, filename_suffix, ".png")),
    plot = p,
    width = 8,
    height = max(6, nrow(msea_plot_data) * 0.3),
    dpi = 600
  )
...
plot_metabolite_class_volcano_plot <- function(msea_res, contrast, dir, padj_cutoff = 0.05, font_size = 16, filename_suffix = "") {
  if (is.null(msea_res) || nrow(msea_res) == 0) {
    message("No MSEA results available for ", contrast)
    return(NULL)
  }
  
  # Add -log10(padj) and significance category
  volcano_data <- msea_res %>%
    mutate(
      neglog10_padj = -log10(pmax(padj, .Machine$double.xmin)),
      Significance = case_when(
        padj < padj_cutoff & NES > 0 ~ "Up",
        padj < padj_cutoff & NES < 0 ~ "Down",
        TRUE ~ "Not Sig."
      ),
      Significance = factor(Significance, levels = c("Up", "Down", "Not Sig.")),
      pathway_short = ifelse(nchar(pathway) > 40, 
                             paste0(substr(pathway, 1, 37), "..."), 
                             pathway)
    ) %>%
    filter(is.finite(neglog10_padj), is.finite(NES))
  
  if (nrow(volcano_data) == 0) {
    message("No valid data points for volcano plot: ", contrast)
    return(NULL)
  }
  ...
  # Create volcano plot
  p <- ggplot(volcano_data, aes(x = NES, y = neglog10_padj, 
                                 size = size, color = Significance, label = pathway_short)) +
    geom_point(alpha = 0.7) +
    scale_color_manual(values = colors, drop = FALSE) +
    scale_size_continuous(range = c(2, 10), name = "Metabolite\nCount") +
    ...
    ggrepel::geom_text_repel(
      data = volcano_data %>% filter(Significance != "Not Sig."),
      size = font_size * 0.3,
      color = "black",
      max.overlaps = 20,
      box.padding = 0.5,
    ...
    labs(
      title = paste0("MSEA Volcano Plot - ", contrast),
      x = "Normalized Enrichment Score (NES)",
      y = "-log10(p-adj)",
      color = "Significance"
    ) +
  ...
  # Save plot
  ggsave(
    filename = file.path(dir, paste0("Volcano_Plot_Metabolite_Classes_", contrast, filename_suffix, ".png")),
    plot = p,
    width = 10,
    height = 8,
    dpi = 600
...
generate_metabolites_volcano_plot <- function(data, pval_col, beta_col, contrast,
                         p_thresh=0.1, beta_thresh=1.0, output_dir="output", feature_type="Metabolite", font_size=20, line_width=3, tick_length=10, point_size=4, label_col=NULL, n_label_limit = 50, filename_suffix="", y_top_padding=0.1, y_max_cap=NULL) {
    ###
    # Generate a volcano plot for metabolite association analysis.
    # Parameters:
    # - data: DataFrame containing the association data
    # - pval_col: Column name for p-values
    # - beta_col: Column name for beta coefficients
